...
import cn.thtns.test.auto.utils.TimeParser;
		...
		public static class ProfitListDTO {
			...
			public static class Profit {
				...
				public static class TimeAndProfit {
					...
					// 解析后的标准化时间对象（LocalDate/YearMonth等）
					private TemporalAccessor normalizedTime;

					// 时间格式类型（DAY/MONTH/SEASON/YEAR）
					private TimeFormat timeFormat;

					// 初始化时自动解析时间和类型
					public void parseTime() {
						this.timeFormat = TimeParser.detectFormat(time);
						this.normalizedTime = TimeParser.parseTime(time);
					}
